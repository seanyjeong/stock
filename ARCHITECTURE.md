...
- **단타 실패 패턴 분석 기반 7개 필터 추가:**
  - 볼륨 1.0x 미만 즉시 제외
  - RSI 과매수(>80) 패널티 -5~-10점
  - 갭앤페이드 필터 (갭>30% + RSI>70 제외)
  - 최소 점수 30→40 상향
  - R:R 비율 1.5:1 미만 제외
  - 핀 바 반등 감지 +3점 보너스
- **투자 성향별 손절폭 cap**: 안정(5%/10%/15%), 균형(8%/15%/20%), 공격(12%/20%/25%) - 단타/스윙/장기
- `lib/base.py`에 `get_profile_type()`, `get_stop_cap()` 추가
- 스윙/장기 스캐너에도 성향별 손절 cap 적용
- **data-status API 수정**: v2 구형 테이블 → v3 daily_scan_results 조회
...
- **환율 수집 개선**: Google Finance → yfinance (`USDKRW=X`) 변경
- **네비게이션 히스토리 수정**: 모든 `goto()` 리다이렉트에 `replaceState: true` 적용, 뒤로가기 정상화
- **투자성향 계산 알고리즘 수정**: 점수 기준 재조정 (0-3/4-7/8-11), `moderate` 수익기대 점수 1→2
